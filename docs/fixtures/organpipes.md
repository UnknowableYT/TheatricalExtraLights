# Organ Pipes (Water Jet Effect)

`theatricalextralights:organpipes` · family: [Water jets](/fixtures/water-jets)

Row of pipes of increasing height; the spread is adjustable.

- Height, thickness and, where available, cone angle or spread are set in the fixture screen (right-click), not on DMX.

## 1-Channel Mode (1 ch)

Height, thickness and, where available, cone angle or spread are set in the fixture screen, not on DMX.

| Ch | Function | Values |
|---|---|---|
| 1 | Jet level | 0 off to 255 full |

### Channel by channel

**1 · Jet level** — Turns the water on and sets its level. 0 is off; higher values raise the flow, which makes the jet taller and denser up to the height set in the fixture screen.
